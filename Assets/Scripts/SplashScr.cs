...
public class SplashScr : mScreen
{
	...
	public override void paint(mGraphics g)
	{
		...
		if (nData != -1)
		{
			if (imgLoadingBN == null)
			{
				try {
					imgLoadingBN = GameCanvas.loadImageRMS("/GifMenu/background.png");
				} catch (Exception ex) {
				}
			}
			...
			if (imgLoadingBN != null)
			{
				g.drawImage(imgLoadingBN, GameCanvas.w / 2, GameCanvas.h / 2, 3);
			}
			else
			{
		...
		{
			// Try to draw background here too
			if (imgLoadingBN == null)
			{
				try {
					imgLoadingBN = GameCanvas.loadImageRMS("/GifMenu/background.png");
	...
	public static Image imgLoadingBN;
